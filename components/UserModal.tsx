...
const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose, onSave, user, brands, currentUserRole }) => {
  ...
  const handleSubmit = () => {
    ...
    if (role === "brand_owner" && ownedBrandIds.length === 0) { alert("Vui lòng chọn ít nhất 1 brand cho Brand Owner"); return; }
    onSave({
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input className={inputClass} value={name} onChange={e => setName(e.target.value)} placeholder="VD: Nguyễn Văn A" />
            </div>
            <div>
              <label className={labelClass}>Email</label>
              <input type="email" className={`${inputClass} ${user ? 'opacity-70 cursor-not-allowed bg-slate-100' : ''}`} value={email} onChange={e => setEmail(e.target.value)} placeholder="user@moodbiz.vn" disabled={!!user} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <select className={`${inputClass} appearance-none cursor-pointer`} value={role} onChange={e => setRole(e.target.value as User['role'])} disabled={currentUserRole === 'brand_owner'}>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-2 max-h-48 overflow-y-auto custom-scrollbar">
                {brands.length === 0 ? <p className="text-sm text-slate-400 p-3 italic">Chưa có brand nào trong hệ thống</p> : brands.map(brand => (
                  <label key={brand.id} className={`flex items-center gap-3 cursor-pointer p-3 rounded-lg transition-colors ${ownedBrandIds.includes(brand.id) ? 'bg-white shadow-sm border border-slate-100' : 'hover:bg-white hover:shadow-sm'}`}>
                    ...
                    <input type="checkbox" checked={ownedBrandIds.includes(brand.id)} onChange={() => handleToggleOwnedBrand(brand.id)} className="hidden" />
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-2 max-h-48 overflow-y-auto custom-scrollbar">
                {brands.length === 0 ? <p className="text-sm text-slate-400 p-3 italic">Chưa có brand nào trong hệ thống</p> : brands.map(brand => (
                  <label key={brand.id} className={`flex items-center gap-3 cursor-pointer p-3 rounded-lg transition-colors ${assignedBrandIds.includes(brand.id) ? 'bg-white shadow-sm border border-slate-100' : 'hover:bg-white hover:shadow-sm'}`}>
                    ...
                    <input type="checkbox" checked={assignedBrandIds.includes(brand.id)} onChange={() => handleToggleBrand(brand.id)} className="hidden" />
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 z-10">
          <button onClick={onClose} className="px-5 py-2.5 text-sm font-bold rounded-xl text-slate-600 hover:bg-slate-200 transition-colors">Hủy bỏ</button>
          <button onClick={handleSubmit} className="px-6 py-2.5 text-sm font-bold rounded-xl bg-[#102d62] text-white hover:bg-blue-900 shadow-lg shadow-blue-900/20 transition-all">
